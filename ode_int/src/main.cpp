...
int main()
{
    ...
    constexpr size_t XPTS = 101;      // grid pts in x-direction
    constexpr size_t TIMESTEPS = 600; // pts in t-direction (time steps)

    // grid coordinates t
    std::vector<double> t_mem =
        hd::get_grid(hd::grid_t::equidistant, hd::rng_t{0.0, 6.0}, TIMESTEPS);
    auto t = mdspan<double, extents<size_t, TIMESTEPS>>(t_mem.data());

    // grid coordinates x
    std::vector<double> x_mem =
        hd::get_grid(hd::grid_t::equidistant, hd::rng_t{0.0, 10.0}, XPTS);
    auto x = mdspan<double, extents<size_t, XPTS>>(x_mem.data());

    // memory for u (t_j, x_i)
    std::vector<double> u_mem(XPTS, 0.0);
    auto u = mdspan<double, extents<size_t, XPTS>>(u_mem.data());

    // memory for rhs = -c*dudx + d*d2udx2 at the four RK steps (space operator)
    std::vector<double> rhs_mem(XPTS, 0.0);
    auto rhs = mdspan<double, extents<size_t, XPTS>>(rhs_mem.data());

    // helper for time integration (idx 0: at t, idx: summation field)
    std::vector<double> uh_mem(2 * XPTS, 0.0);
    auto uh = mdspan<double, extents<size_t, 2, XPTS>>(uh_mem.data());

    // memory for dudx
    std::vector<double> dudx_mem(XPTS, 0.0);
    auto dudx = mdspan<double, extents<size_t, XPTS>>(dudx_mem.data());

    // memory for d2udx2
    std::vector<double> d2udx2_mem(XPTS, 0.0);
    auto d2udx2 = mdspan<double, extents<size_t, XPTS>>(d2udx2_mem.data());

    // coefficients for f  (x_i) in stencil for f'
    std::vector<double> a0_f1_mem(XPTS), b0_f1_mem(XPTS), c0_f1_mem(XPTS);
    auto a0_f1 = mdspan<double, extents<size_t, XPTS>>(a0_f1_mem.data());
    auto b0_f1 = mdspan<double, extents<size_t, XPTS>>(b0_f1_mem.data());
    auto c0_f1 = mdspan<double, extents<size_t, XPTS>>(c0_f1_mem.data());

    // coefficients for f' (x_i) in stencil for f'
    // std::vector<double> a1_f1_mem(XPTS), b1_f1_mem(XPTS), c1_f1_mem(XPTS);
    // auto a1_f1 = mdspan<double, extents<size_t, XPTS>>(a1_f1_mem.data());
    // auto b1_f1 = mdspan<double, extents<size_t, XPTS>>(b1_f1_mem.data());
    // auto c1_f1 = mdspan<double, extents<size_t, XPTS>>(c1_f1_mem.data());

    // coefficients for f  (x_i) in stencil for f''
    std::vector<double> a0_f2_mem(XPTS), b0_f2_mem(XPTS), c0_f2_mem(XPTS);
    auto a0_f2 = mdspan<double, extents<size_t, XPTS>>(a0_f2_mem.data());
    auto b0_f2 = mdspan<double, extents<size_t, XPTS>>(b0_f2_mem.data());
    auto c0_f2 = mdspan<double, extents<size_t, XPTS>>(c0_f2_mem.data());

    // coefficients for f''(x_i) in stencil for f''
    // std::vector<double> a2_f2_mem(XPTS), b2_f2_mem(XPTS),
    //     c2_f2_mem(XPTS); // coefficients for f''(x_i)
    // auto a2_f2 = mdspan<double, extents<size_t, XPTS>>(a2_f2_mem.data());
    // auto b2_f2 = mdspan<double, extents<size_t, XPTS>>(b2_f2_mem.data());
    // auto c2_f2 = mdspan<double, extents<size_t, XPTS>>(c2_f2_mem.data());
